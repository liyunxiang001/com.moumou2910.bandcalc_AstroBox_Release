...
一款运行于小米穿戴设备（Vela 快应用 / 手环 10 Pro）上的科学计算器。支持基础四则运算、科学函数、位运算，并提供可开关的二进制/十六进制结果对照。

> 包名：`com.moumou2910.bandcalc`
> 目标设备：小米手环 10 Pro（矩形屏 336×480）
...
- **官方 debug 通道**：手机「小米运动健康」→ 我的 → 关于 → Debug → 第三方应用 → Install third app → 选择 `.rpk`。
- **社区 ADB 方式**：手环用充电线连电脑，进开发者/ADB 模式推装 `.rpk`（参考米坛 BandBBS 教程）。

> ⚠️ AIoT IDE 的「真机调试」目前仅支持 Xiaomi Watch S4，手环 10 Pro 暂不可用。
